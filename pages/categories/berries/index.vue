<template lang="pug">
.category
    Breadcrumbs(:razdel="razdel" currentPage="Ягоды")
    .category-info
        h1.title Заказывайте ягоды с быстрой и удобной доставкой на дом 
    .category-flex
        ProductsWrapper(:products="berries" folder="categories" category="berries")
  ...
  <script>
    export default{
      ...
      async fetch ({ store }) {
        await store.dispatch('getBerries')
      },
      head(){
        return {
          title: '🫐 Доставка ягод на дом по Москве',
          meta: [
            { charset: 'utf-8' },
            { name: 'viewport', content: 'width=device-width, initial-scale=1' },
            {
              hid: 'description',
              name: 'description',
              content: 'Заказывайте ягоды в нашем интернет магазине. Мы обеспечим быструю и удобную доставку. У нас большой ассортимент ягод на любой вкус и цвет'
            }
          ],
        }
      },
