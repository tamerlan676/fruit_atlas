<template lang="pug">
.category
    Breadcrumbs(:razdel="razdel" currentPage="Экзотика")
    .category-info
        h1.title Заказывайте экзотические фрукты с быстрой и удобной доставкой на дом 
    .category-flex
        ProductsWrapper(:products="exotics" folder="categories" category="exotics")
  ...
  <script>
    export default{
      ...
      async fetch ({ store }) {
        await store.dispatch('getExotics')
      },
      head(){
        return {
          title: '🥥 Доставка экзотических фруктов по Москве',
          meta: [
            { charset: 'utf-8' },
            { name: 'viewport', content: 'width=device-width, initial-scale=1' },
            {
              hid: 'description',
              name: 'description',
              content: 'Заказывайте экзотические фрукты в нашем интернет магазине. Мы обеспечим быструю и удобную доставку. Вам останется лишь насладиться вкусом.'
            }
          ],
        }
      },
      ...
      computed: {
        exotics() {
          return this.$store.state.exotics
        },
        cart() {
