...
.inner-product
    ...
    .product-wrapper
        ...
        .info
            ...
            .message-about-change Возможно добавление/замена продукта на Ваш выбор
            .message-about-change {{ getItem.acf.country }}
...
export default {
      ...
      iconsInfo: [
        {
          ...
          text: `<span>Доставка</span>: Привезем корзину сегодня в течении 2-3 часов.`
        },
        {
          img: require("~/assets/images/baskets-icons/gift.svg"),
          text: `<span>Упаковка</span>: Мы упаковываем корзины с фруктами в прозрачную пленку и украшаем бантом из рафии ручной работы.`
        },
        {
          img: require("~/assets/images/baskets-icons/quality.svg"),
          text: `<span>Гарантия качества</span>: Мы тчательно подбираем каждый фрукт в вашу корзину. Выбираем самоее свежее и красивое.`
        },
        {
  ...
  computed: {
    getItem() {
      return this.$store.state.baskets.filter(item => item.id === +this.$route.params.id)[0]
    }
  },
  methods: {
    addToCart(product) {
